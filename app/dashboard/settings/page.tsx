...
const achievements = [
  {
    id: 1,
    category: "Score",
    message: "Congratulations! You achieved a score of 6.5. To get a better result, please exercise more.",
    status: "success",
  },
  {
    id: 2,
    category: "Streak",
    message: "Amazing! You've logged in for 7 days straight. Consistency is the key to mastering new skills.",
    status: "info",
  },
  {
    id: 3,
    category: "Reminder",
    message: "Your subscription is expiring in 3 days. Renew now to keep your progress synced.",
    status: "warning",
  },
  {
    id: 4,
    category: "Milestone",
    message: "You completed the 'Advanced Grammar' module with an 85% accuracy rate. Level up!",
    status: "success",
  },
  {
    id: 5,
    category: "Feedback",
    message: "Your tutor left a comment: 'Great job on the essay, but watch your punctuation in complex sentences.'",
    status: "neutral",
  },
  {
    id: 6,
    category: "System",
    message: "Maintenance scheduled for Saturday at 2:00 AM UTC. Expect brief interruptions.",
    status: "error",
  },
  {
    id: 7,
    category: "Badge",
    message: "New Badge Unlocked: 'Vocabulary Voyager' for learning 500 unique words this month.",
    status: "success",
  },
  {
    id: 8,
    category: "Goal",
    message: "You are only 200 points away from reaching the Platinum Tier. Keep pushing!",
    status: "info",
  },
  {
    id: 9,
    category: "Challenge",
    message: "Weekend Warrior challenge is live! Complete 3 mock tests to earn double XP.",
    status: "warning",
  },
  {
    id: 10,
    category: "Score",
    message: "Diagnostic test complete. Your current level is estimated at B2 (Upper Intermediate).",
    status: "neutral",
  },
];
    ...
    <DashboardShell className="max-w-[1200px] p-6 lg:p-10 space-y-8">
      <header>
        ...
        <p className="text-muted-foreground text-lg">
          Manage your account preferences, security configurations, and API keys.
        </p>
      </header>
          ...
          {activeButton === 0 && (
            <Card className="border-border/50 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-950/40 backdrop-blur-xl shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Your profile details are synced securely with your active authentication session.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="flex items-center gap-4 p-4 rounded-2xl bg-zinc-50/50 dark:bg-zinc-900/50 border border-border/10">
                  ...
                  <div>
                    <h4 className="font-bold text-zinc-900 dark:text-zinc-100">{displayName}</h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground/80">First Name</Label>
                    <Input disabled value={firstName} className="bg-zinc-50/50 dark:bg-zinc-900/50 text-muted-foreground cursor-not-allowed" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground/80">Last Name</Label>
                    <Input disabled value={lastName} className="bg-zinc-50/50 dark:bg-zinc-900/50 text-muted-foreground cursor-not-allowed" />
                  </div>
                </div>

                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground/80">Email Address</Label>
                  <Input disabled value={user?.email || ""} className="bg-zinc-50/50 dark:bg-zinc-900/50 text-muted-foreground cursor-not-allowed" />
                </div>
              </CardContent>
            </Card>
          )}
              ...
              <CardContent className="space-y-5">
                ...
                {apiSaveStatus === "success" && (
                  <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-3 text-emerald-500">
                    ...
                    <span className="text-xs font-bold">API Connection Key verified and saved successfully!</span>
                  </div>
                )}
                {apiSaveStatus === "error" && (
                ...
                >
                  {isSavingApi ? "Saving Connection..." : "Save Connection"}
                </Button>
                  ...
                  <Table>
                    <TableHeader className="bg-zinc-100/50 dark:bg-zinc-900/50">
                      <TableRow>
                        ...
                        <TableHead className="w-24 font-bold">Category</TableHead>
                    ...
                    <TableBody>
                      {achievements.map((item) => (
                        <TableRow key={item.id} className="hover:bg-zinc-100/30 dark:hover:bg-zinc-900/10">
                          ...
                          <TableCell className="text-sm font-medium">
                            <p className="max-w-[320px] lg:max-w-[420px] truncate" title={item.message}>
                              {item.message}
                            </p>
                          </TableCell>
          ...
          {activeButton === 3 && (
            <div className="space-y-6">
              ...
              <Card className="border-border/50 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-950/40 backdrop-blur-xl shadow-sm">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground/80">Current Password</Label>
                    <Input type="password" placeholder="••••••••" className="bg-white/50 dark:bg-zinc-900/50 border-border/50 dark:border-zinc-800/80 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground/80">New Password</Label>
                    <Input type="password" placeholder="••••••••" className="bg-white/50 dark:bg-zinc-900/50 border-border/50 dark:border-zinc-800/80 rounded-xl" />
                  </div>
                  <Button variant="blue" className="w-full sm:w-auto font-bold cursor-pointer">Update Password</Button>
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between p-4 rounded-xl bg-zinc-50/50 dark:bg-zinc-900/50 border border-border/10">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-sm">Mac OS - Chrome Browser</h4>
                        <p className="text-xs text-muted-foreground">Istanbul, Turkey • Active Now</p>
                      </div>
                    </div>
