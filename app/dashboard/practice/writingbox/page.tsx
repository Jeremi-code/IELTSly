...
// ── TaskFigure: Displays IELTS Task 1 figure (diagram, chart, map, etc.) ──
function TaskFigure({
  imageUrl,
    ...
    <div className="rounded-2xl overflow-hidden border border-border/60 dark:border-zinc-700/70 bg-white dark:bg-zinc-900/90 shadow-sm space-y-0">
      ...
      <div className="flex items-center justify-between px-3.5 py-2 border-b border-border/40 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/90">
        <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
          <ImageIcon className="h-3.5 w-3.5 text-primary" /> Visual Figure /
          Diagram
        </span>
        <a
          href={imageUrl}
      ...
      <div
        className={cn(
          "p-2 sm:p-3 flex items-center justify-center bg-zinc-50/40 dark:bg-zinc-950/40 overflow-y-auto",
          maxHeightClass,
        )}
      >
        ...
        <img
          ...
          loading="eager"
          className="w-full object-contain rounded-xl drop-shadow-xs"
        />
      </div>
    </div>
  );
