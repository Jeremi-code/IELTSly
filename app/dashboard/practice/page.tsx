      ...
      <main className={cn(
        ...
        <div className="max-w-[1200px] mx-auto p-6 lg:p-12 space-y-12">
          ...
          <header className="space-y-4 text-center">
            ...
            >
              Choose your path: refine specific skills in practice mode or take a full exam to assess your overall performance.
            </motion.p>
          </header>
              ...
              <motion.div key={mode.title} variants={item}>
                <Card className="group relative overflow-hidden h-full border-border/50 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-900/40 backdrop-blur-xl shadow-xl hover:shadow-2xl transition-all duration-500 hover:border-primary/30">
                  ...
                  <CardHeader className="space-y-4">
                    <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center transition-transform duration-500 group-hover:scale-110 group-hover:rotate-12", mode.bg)}>
                      <mode.icon className={cn("h-8 w-8", mode.color)} />
                    </div>
                    <div>
                      <CardTitle className="text-3xl font-bold">{mode.title}</CardTitle>
                      <CardDescription className="text-lg mt-2">{mode.description}</CardDescription>
                    </div>
                  </CardHeader>
                  ...
                  <CardContent className="space-y-8">
                    <div className="space-y-4">
                      {mode.features.map((feature, fIdx) => (
                        <div key={fIdx} className="flex items-center gap-3 text-muted-foreground group/feature">
                ...
                <div className="flex flex-col lg:flex-row items-center">
                  ...
                  <div className="px-8 py-6 lg:border-r border-border/10 dark:border-zinc-800/20 bg-primary/5 flex items-center gap-4">
                    ...
                    <div className="whitespace-nowrap">
                      <h3 className="text-sm font-black tracking-tight uppercase leading-none">Live Mastery</h3>
                      <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-1 opacity-60">Insight Console</p>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 w-full">
                    {progressStats.map((stat, idx) => (
                      <div key={idx} className="px-8 py-6 flex lg:flex-col items-center justify-center gap-2 group/stat border-r last:border-0 border-border/10 dark:border-zinc-800/10 hover:bg-black/5 dark:hover:bg-white/5 transition-all duration-300">
                        <stat.icon className="h-3.5 w-3.5 text-muted-foreground group-hover/stat:text-blue-500 transition-colors" />
                        <div className="flex flex-col items-center">
                          <span className="text-2xl font-black tracking-tighter group-hover/stat:scale-110 transition-transform duration-300">{stat.value}</span>
                          <span className="text-[9px] uppercase tracking-[0.2em] font-black text-muted-foreground/40 group-hover/stat:text-primary transition-colors">{stat.label}</span>
                        </div>
                      </div>
                    ))}
                  </div>
